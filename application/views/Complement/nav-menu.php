<aside id="left-panel">
	...
	<div class="login-info">
		<span> <!-- User image size is adjusted inside CSS, it should stay as it --> 
			...
			<a href="javascript:void(0);" id="show-shortcut" data-action="toggleShortcut">
				<img src="<?=$_SESSION['ImgUser'];?>" alt="me" class="online" /> 
	...
	<nav>
		...
		<?php
		  if(isset($getSecciones)){
		    $lastSeccion='';
		    $contSecciones=1;
		    foreach ($getSecciones as $gs) {
		      
		      if($lastSeccion!=$gs->Seccion){
		        if($contSecciones>1){
		          ?>
		            </ul>
		          </li>
		        ...
		        $contSecciones++;
		        $lastSeccion=$gs->Seccion;

		        $classActiveSection='';
		        $arrayModulo=explode('/',$gs->UrlModulo);

		        if($this->uri->segment("1")==$arrayModulo[0]){
		        	$classActiveSection='active';
		        }
		      ?>
		       
		       <li>
		       	<a href="#" title="<?=$gs->Seccion;?>">
		       		<i class="fa fa-lg fa-fw <?=$gs->IcoSeccion;?>"></i> 
		       		<span class="menu-item-parent"><?=$gs->Seccion;?></span></a>
					<ul>
		      ...
		      <?php    
		       ...
		       $checkedModulo='';  
		        if($gs->IdUsuario!=''){
		        	$classActiveModulo='';
			        if($this->uri->segment("1").'/'.$this->uri->segment("2")==$gs->UrlModulo){
			        	$classActiveModulo='active';
			        }
		        ?>
				<li class="<?=$classActiveModulo;?>">
					<a href="<?=base_url().$gs->UrlModulo;?>"> 
						<i class="fa fa-fw <?=$gs->Ico;?>"></i> 
						<?=$gs->Modulo;?>
